...
echo "🔧 Исправление импортов в моделях..."
cd ~/legends-of-elara/backend

# 1. enums.py (без изменений, но проверим)
cat > app/models/enums.py <<'EOF'
...
# 3. character.py (с полными импортами)
cat > app/models/character.py <<'EOF'
import uuid
from datetime import datetime
...
echo "✅ Все модели обновлены с корректными импортами."